...
export function CreditPurchaseForm({ isSubscription }) {
  ...
  const { data: subData } = useSWR("/api/stripe/subscription", (url) =>
  ...
  const proceedToStripe = async () => {
    ...
    if (data?.url) {
      if (data.url.startsWith("/")) {
        // Redirection locale (mise à jour abonnement)
        popup.close();
  ...
  const handleCheckout = async () => {
    if (
      isSubscription &&
      activeSub &&
      selectedPack.comments !== activeSub.quantity
    ) {
      setShowUpgrade(true);
      return;
    }
  ...
  return (
    <>
        ...
        <div className="bg-[#F6F6F6] p-6 md:p-8 flex flex-col gap-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Total number of comments you’ll be able to allocate across ads.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {PRESET_PACKS.map((pack) => {
              const isSelected =
                selectedPack.comments === pack.comments && !customCount;
              const isCurrentPlan =
                isSubscription &&
                activeSub &&
                pack.comments === activeSub.quantity;
              const savings = Math.round(
                ((1.25 - pack.pricePerComment) / 1.25) * 100
              );
              ...
              return (
                ...
                >
                  ...
                  {isCurrentPlan && (
                    <div className="absolute bottom-1 text-[10px] text-blue-600">
                      Current plan
                    </div>
                  )}
                </button>
          ...
          <div className="bg-white rounded-xl border p-6">
            ...
            <div className="grid grid-cols-2 gap-y-3 gap-x-8 text-sm text-gray-800">
              <div className="flex items-start gap-2">
                <Check className="text-[#1870EC] w-4 h-4" /> Human-written
                comments
              </div>
              <div className="flex items-start gap-2">
                <Check className="text-[#1870EC] w-4 h-4" /> Real profiles only
              </div>
              <div className="flex items-start gap-2">
                <Check className="text-[#1870EC] w-4 h-4" /> Persona-matched
                replies
              </div>
              <div className="flex items-start gap-2">
                <Check className="text-[#1870EC] w-4 h-4" /> Fast delivery
              </div>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span>{selectedPack.comments}</span>
              </div>
              <div className="flex justify-between">
                <span>Price/comment</span>
                <span>${unitAmount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between">
                <span>Sub-total</span>
      ...
      <UpgradeDialog
        ...
        currentQuantity={activeSub?.quantity}
        newQuantity={selectedPack.comments}
        newPrice={total}
      />
    </>
  );
